...
$rank_slug = $_GET['slug'];
$rank = getRankBySlug($rank_slug);

/* Set the page title */
$page_title = $rank->long_name ."s - Guild Roster";
...
?><ul id="breadcrumbs">
		...
		<li><?php echo $rank->long_name; ?>s</li>
	</ul>
...
if( $roster = $db->query("SELECT `id` FROM `characters` WHERE `rank` = ". $rank->id ." ORDER BY `name`") ) {
