...
if( $roster = $db->query("SELECT `id` FROM `characters` ORDER BY `rank`, `name`") ) {
	...
	?><!-- Roster table -->
	<table id="rosterTable" class="tablesorter">
		<thead>
			<tr>
	...
	<?php
		...
		while($member = $roster->fetch_object()) {
			...
			?><tr>
				...
				<td class="<?php echo $rank->id; ?>"><?php switch($rank->alt_status) {
				
					case 0:
						?><a href="/roster/rank/<?php echo $rank->slug; ?>" title="Click to view all the <?php echo $rank->long_name; ?>s"><?php echo $rank->long_name; ?></a><?php
						break;
					
					case 1:
						?><a href="/roster/alts/<?php echo $rank->slug; ?>" title="Click to view all the <?php echo $rank->long_name; ?>s"><?php echo $rank->long_name; ?></a><?php
						break;
						
					} ?></td>
				<td><?php echo $character->achievements; ?></td>
				<?php if( isset($account) && empty($character->account_id) ) {
					?><td><a href="/roster/character/<?php echo $character->name; ?>/claim" title="Claim this character">Claim</a></td><?php
				} ?>
			</tr>
<?php	} ?>
		</tbody>
